...
class MlTrainer:
    ...
    def __data_prep(self, df_bkg, df_prompt, df_nonprompt, pt_bin, out_dir, bkg_factor):
        ...
        if 0 < self.test_frac < 1:
            train_set, test_set, y_train, y_test = train_test_split(df_tot, labels_array, test_size=self.test_frac, random_state=self.seed_split)
        else:
    ...
    def __train_test(self, train_test_data, hyper_pars, pt_bin, out_dir):
        ...
        plt.clf()
        testplotx = bdt_efficiency_array(train_test_data[3], y_pred_test)[0]
        testploty = bdt_efficiency_array(train_test_data[3], y_pred_test)[1]
        plt.plot(testplotx, testploty, 'o')
        namept = str(pt_bin)
        namept = namept.replace("[","")
        namept = namept.replace("]","")
        namept = namept.replace(" ","")
        namept = namept.replace(",","-")
        namept = 'Output_Training/Pictures/Test/bdt_efficiency' + namept + '.png'
        plt.savefig(namept)
